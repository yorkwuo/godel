  ...
  for (var k = 0; k < tables.length; k++) {
      ...
      for (var i = 0; i < cells.length; i++) {
        ...
        if (colname === "DEL") {
          ...
          cells[i].addEventListener('click', function(){
            var gname   = this.getAttribute("gname");
            window.location = gname + "/.index.html"
          })
        } else {
          cells[i].addEventListener('click', function(){
            this.style.backgroundColor = "lightyellow";
...
function lsearch() {
  ...
  .then(res => {
    ...
    for (let i in result) {
      ...
      txt += `<div><a href="${path}/.index.html">${pagename}<br><img height=50px src="${path}/cover.png"></img><a></div>`
    }
    txt += '</div>'
    p.innerHTML = txt
  })
...
function jumpto(row) {
  console.log(row)
  window.location.href = row + '/.index.html'
}

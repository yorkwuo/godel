...
# area_normalize
  if [info exist vars(module)] {
    ...
    if [info exist define($vars(module),unit_area)] {
      ...
      puts "    Error: No \$define($vars(module),unit_area). Use default unit_area: 5000"
      set define($vars(module),unit_area) 5000
    }

    set unit $define($vars(module),unit_area)
    if [info exist vars(area)] {
      set vars(area_normalize) [format "%.2f" [expr $vars(area)/$unit.0]]
    }
  }
# power efficiency
  if {[info exist vars(current_speed)] && [info exist vars(power_total)]} {
    set vars(PE) [format "%.3f" [expr $vars(power_total)/$vars(current_speed).0]]
  }
}

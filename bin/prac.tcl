...
proc nextone {} {
  ...
  set item [lindex $flist $random_one]
  set flist [lreplace $flist $random_one $random_one]

  .fr.word    configure -text "$item"
  set kin [open $item/q.md r]
    set question [read $kin]
  close $kin
  .fr.question configure -text "$question"
...
proc next2 {} {
  ...
  .fr.question configure -text $vars($ifile,question)
  .fr.example configure -text ""
...
proc hint {} {
  ...
  set kin [open $::curword/a.md r]
  set right_answer [read $kin]
  close $kin
...
proc disp_chinese {} {
  ...
  .fr.question configure -text $vars($curword,question)
...
proc filter_out {} {
  ...
  set root $dicroot
  set chi [lvars $root/$filter question]
  if {$chi eq "NA"} {
    set ::question ""
    .fr.txt delete 1.0 end
  } else {
  ...
  while {[gets $kin line] >= 0} {
    if [regexp {^\s*#} $line] {
      ...
      if [file exist $dicdir/$w] {
        set question [lvars $dicdir/$w question]
        puts $kout "lappend allwords $w"
        puts $kout "set vars($w,path) $dicdir/$w"
        puts $kout "set vars($w,question) \"$question\""    
        puts $kout "set vars($w,concat) \"$w $question\""
      } else {
        puts "Not exist... $w"
      }
    }
...
proc check_answer {} {
  ...
  if [regexp "\\y$::answer\\y" $right_answer] {
    incr ::correct
    .fr.word    configure  -text $right_answer
    #.fr.example configure  -text $right_answer
    set ::answer ""
  } else {
    incr ::wrong
  }
...
label .fr.question -text "na" -justify left -wraplength 500 -pady 10
grid  .fr.question -row 5 -column 0  -sticky w
